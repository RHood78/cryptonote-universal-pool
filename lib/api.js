...
function getAverage(hashrateArr){
    if(hashrateArr.length == 0) {
        return 0;
    }

    total = 0;
    for(var i = 0; i < hashrateArr.length; i++){
        total += hashrateArr[i][1];
    }
    return (total / hashrateArr.length);
}
...
function handleMinerStats(urlParts, response){
    ...
    if (urlParts.query.longpoll === 'true'){
        ...
        redisClient.multi([
            ...
            //The charts have a peculiar thing where empty indices are not
            //shown (i.e. the HR was 0 for 60 min it would skip 4 values)
            //TODO: Figure out how to have the 0 value times in the chart.

            charts.getUserChartsData(address, replies[1], function(error, chartsData) {
                stats.usravg = getAverage(chartsData['hashrate'])
        ...
        function(workerKeys, callback) {
            ...
            redisClient.multi(redisCommands).exec(function(error, redisData) {
                ...
                for(var i in redisData) {
                    ...
                    charts.getUserChartsData(address, null, function(error, chartsData) {
                        workersData[address].averageHashrate = getAverage(chartsData['hashrate']);
                    });
                }
